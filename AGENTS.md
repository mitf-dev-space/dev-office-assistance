...
- **LAN egress:** `10.100.235.21` currently has **no outbound public HTTPS** (OpenRouter/ClickUp/GitHub time out; internal GitLab OK). Workspace AI + ClickUp cannot work until firewall allows `:443` or `HTTPS_PROXY` is set — redeploy alone will not fix it. See [`docs/deployment/compose-ai-secrets-pitfalls.md`](docs/deployment/compose-ai-secrets-pitfalls.md).
...
- **Production LAN:** deploys to `10.100.235.21` (web `:46810`, API `:46811`) on the same host as OmniTest; see [`docs/deployment/lan-server-reference.md`](docs/deployment/lan-server-reference.md).
